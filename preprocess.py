...
class TextStandardizeLayer(tf.keras.layers.Layer):
    ...
    def standardization(text_t):
        ...
        for short, full in TextStandardizeLayer.contractions.items():
        ...
        for char in ('a', 'e', 'i', 'o', 'u', 'y', 's', 'h', 'f', 'r', 'm'):
            text_t = tf.strings.regex_replace(text_t,
                                              ...
                                              replace_global=True,
                                              name='fix_streched')
        for chat_word, repl in TextStandardizeLayer.chat_words.items():
            text_t = tf.strings.regex_replace(text_t,
                                              '(?i)[^A-Z]' + chat_word +
                                              '[^A-Z]',
                                              f' {repl} ',
    ...
    def call(self, inputs):
        return TextStandardizeLayer.standardization(inputs)
